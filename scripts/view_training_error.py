...
def get_runs_dir(model):
    subdir = MODEL_TO_SUBDIR[model]
    return Path("generated_models") / subdir


def list_runs(model):
    """List all available training runs for a given model type."""
    runs_dir = get_runs_dir(model)
    if not runs_dir.exists():
        print(f"No runs found for '{model}' in {runs_dir}.")
        return []
...
def view_error_graph(model="transformer", run_path=None, show_latest=False):
    """
    View error graph for a training run.

    Args:
        model: Model type to inspect ('transformer', 'vae'/'encoder', 'mlp'/'predictor')
    ...
    if not runs:
        print(f"No runs found in {get_runs_dir(model)}")
        return
    ...
    if run_path:
        ...
        if not run_path.exists():
            # Allow passing only run folder name, not full path
    ...
    model_label = MODEL_TO_SUBDIR[model]
    print(f"Found {len(runs)} {model_label} training runs:\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="View training error graphs")
    parser.add_argument(
        "--model",
        type=str,
        default="transformer",
        choices=["transformer", "vae", "encoder", "mlp", "predictor"],
        help="Model type: transformer, vae/encoder, or mlp/predictor",
    )
    ...
    view_error_graph(model=args.model, run_path=args.run, show_latest=args.latest)
